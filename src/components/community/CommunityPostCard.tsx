...
export function CommunityPostCard({ post, isAdmin, onDelete, onLike, treatmentName, priority = false }: CommunityPostCardProps) {
    const { user } = useAuth();
    ...
    return (
        <div className="bg-white rounded-[1.5rem] md:rounded-[2.5rem] border-2 border-gray-100 shadow-sm md:shadow-lg overflow-hidden group hover:shadow-2xl hover:border-[#34baab]/20 transition-all duration-300 h-full flex flex-col">
            {/* User Info */}
            ...
            <div className="relative aspect-square w-full overflow-hidden">
                ...
                {treatmentName && (
                    <div className="absolute bottom-2 left-2 right-2 md:bottom-4 md:left-4 md:right-4">
                        <div className="bg-white/80 backdrop-blur-md px-2 py-1 md:px-4 md:py-2 rounded-lg md:rounded-2xl flex items-center gap-1.5 md:gap-2 shadow-lg border border-white/20">
                            <TagIcon className="w-2.5 h-2.5 md:w-4 md:h-4 text-[#34baab]" />
                            <span className="text-[8px] md:text-xs font-black text-gray-900 truncate">{treatmentName}</span>
                        </div>
                    </div>
                )}
            </div>
