...
import { Bell, Send, Users, User, History, Trash2, ArrowLeft } from 'lucide-react';
...
export default function NotificationsPage() {
    ...
    const loadData = async () => {
        try {
            ...
            // Include ALL users in the specific list
                                    ...
                                    >
                                        ...
                                        {clients.map(c => (
                                            <option key={c.uid} value={c.uid}>
                                                {c.fullName} ({c.role === 'client' ? 'Cliente' : c.role === 'professional' ? 'Profesional' : c.role === 'admin' ? 'Admin' : 'Secretaría'})
                                            </option>
                                        ))}
                                    </select>
                                </div>
                                    ...
                                    >
                                        <div className="flex justify-between items-start mb-2">
                                            <div className="flex items-center gap-2">
                                                <span className={`px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-wider ${item.type === 'broadcast'
                                                    ? 'bg-blue-100 text-blue-700'
                                                    : 'bg-amber-100 text-amber-700'
                                                    }`}>
                                                    {item.type === 'broadcast' ? 'Masivo' : 'Individual'}
                                                </span>
